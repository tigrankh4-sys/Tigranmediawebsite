...
import { steden } from './src/data/steden.ts';
...
const css = `
  ...
  @media (max-width: 768px) {
    ...
    .tm-stat-col,
    .tm-stat-col:first-child,
    .tm-stat-col:last-child {
      padding: 0;
      text-align: left;
    }
...
function CounterStat({ target, decimals = 0, suffix = '', label, sub }) {
  ...
  return (
    <div className="tm-stat-col" ref={ref}>
      <span className="tm-stat-num">{display}{suffix}</span>
      <span className="tm-stat-label">{label}</span>
      <span className="tm-stat-sub">{sub}</span>
    </div>
  );
}
...
const plans = [
  {
    name: 'Starter',
    price: '€500',
    sub: 'Voor wie wil starten.',
    features: [
      'Volledig custom Next.js website',
      'Custom CMS + eigen beheersysteem',
      'Live in 1 week',
      '99/100 Google score',
      'SEO technisch correct',
    ],
    cta: 'Kies Starter',
    highlight: false,
  },
  ...
  },
  {
    name: 'Full Service',
    price: '€1.500',
    sub: 'Voor wie wil domineren.',
    features: [
      'Volledig custom Next.js website',
      'Custom CMS + eigen beheersysteem',
      'SEO + copywriting + strategie',
      'Live in 1 week',
      '99/100 Google score',
      '3 maanden support',
    ],
    cta: 'Kies Full Service',
    highlight: false,
  },
];
...
const faqs = [
  {
    q: 'Waarom zijn jullie prijzen zo laag?',
    a: 'Omdat wij slank werken. Geen groot kantoor, geen overhead, geen tussenpersonen. Wij investeren in technologie en vakmanschap — niet in dure bureaukosten. Die besparing geven wij door aan u.',
  },
  {
    q: 'Hoe lang duurt het?',
    a: 'Één week. Vanaf het moment dat wij alle inhoud van u hebben, is uw site live binnen 7 dagen. Geen weken wachten, geen vage deadlines.',
  },
  {
    q: 'Wat krijg ik precies?',
    a: 'Een volledig custom Next.js website — geen template, geen WordPress. Inclusief een eigen beheersysteem waarmee u zelf teksten en afbeeldingen kan aanpassen. Zonder technische kennis.',
  },
  {
    q: 'Heb ik technische kennis nodig?',
    a: 'Nee. Uw beheersysteem is gebouwd voor ondernemers, niet voor developers. Als u een e-mail kan schrijven, kan u uw site beheren.',
  },
  {
    q: 'Wat als ik niet tevreden ben?',
    a: 'Wij werken in nauw overleg. Na de discovery call weet u exact wat u krijgt. Geen verrassingen, geen meerkosten. Als er iets niet klopt, lossen wij het op.',
  },
];
...
function ProjectImage() {
  ...
  if (err) {
    return (
      <div className="tm-project-img-fallback">
        <span>Forest &amp; Bean</span>
      </div>
    );
  }
  return (
    <img
      ...
      src="/forest-bean-preview.jpg"
      alt="Forest & Bean — Coffeeshop website"
      onError={() => setErr(true)}
    />
  );
}
  ...
  return (
    <ul className="tm-faq-list">
      {faqs.map((item, i) => {
        const isOpen = open === i;
        return (
          <li className="tm-faq-item" key={i}>
            <button
...
export default function TigranMediaSections() {
  return (
    <>
      <style>{css}</style>
      ...
      <main>
        {/* STATS COUNTER */}
        <section className="tm-stats-bar">
          <div className="tm-stats-grid">
            <CounterStat target={5.0} decimals={1} suffix="" label="Google Rating" sub="Geverifieerd" />
            <CounterStat target={100} decimals={0} suffix="%" label="Tevreden klanten" sub="Altijd" />
            <CounterStat target={99} decimals={0} suffix="/100" label="Google Score" sub="Gegarandeerd" />
          </div>
        </section>
        ...
        <section id="werkwijze">
          <div className="tm-section">
            ...
            <div className="tm-steps">
              {steps.map((s) => (
                <div className="tm-step" key={s.num}>
          ...
          <div className="tm-section">
            ...
            <div className="tm-pricing-grid">
              {plans.map((p) => (
                <div
                  key={p.name}
                  className={`tm-price-card${p.highlight ? ' tm-price-card--highlight' : ''}`}
                >
            ...
            <div className="tm-contact-inner">
              ...
              <form className="tm-form" onSubmit={(e) => e.preventDefault()}>
                <div className="tm-field">
                  <label>Naam</label>
                  <input type="text" placeholder="Jan Janssen" />
                </div>
                <div className="tm-field">
                  <label>Bedrijf</label>
                  <input type="text" placeholder="Jouw Bedrijf BV" />
                </div>
                <div className="tm-field">
                  <label>E-mail</label>
                ...
                <div className="tm-field">
                  <label>Bericht</label>
                  <textarea rows={4} placeholder="Vertel ons over uw project..." />
                </div>
                <button type="submit" className="tm-submit">Verstuur →</button>
              </form>
            </div>
          </div>
        </section>
